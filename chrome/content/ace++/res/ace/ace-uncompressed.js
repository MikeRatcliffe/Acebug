...
(function() {
    ...
    this.moveCursorBy = function(rows, chars) {
        var row = this.selectionLead.row;
        var col = this.selectionLead.column;

        var screenPos = this.session.documentToScreenPosition(row, col);
        var screenCol = (chars == 0 && this.$desiredColumn) || screenPos.column;
        var docPos = this.session.screenToDocumentPosition(screenPos.row + rows, screenCol);

        // if we are at the EOL of current line, and targetline is all whitespace
        // set the new column to the EOL of the target line, without updating desired column
        // TODO: will this be better handled in moveCursorUP/Down?
        if (chars == 0) {
            var currentLine = this.session.getLine(row);
            var targetLine = this.session.getLine(docPos.row);
            if (
                currentLine && targetLine
                && currentLine.length === col
                && targetLine.match(/^\s*$/)
            ) {
                docPos.column = targetLine.length;
            }
        }
